...
function validateJournal(body: any) {
  ...
  for (const [i,line] of lines.entries()) {
    ...
    if (debit < 0 || credit < 0) errors.push(`Line ${i + 1}: debit and credit cannot be negative.`);
  ...
  const debitTotal = round(lines.reduce((s: number,l: any)=>s+Number(l.debit??0),0));
  const creditTotal = round(lines.reduce((s: number,l: any)=>s+Number(l.credit??0),0));
...
function normal(a:any){return a.accountType==='asset'||a.accountType==='expense'?round(Number(a.debit??0)-Number(a.credit??0)):round(Number(a.credit??0)-Number(a.debit??0));}
function section(accounts:any[], prefix:string, type:string){const lines=accounts.filter(a=>a.accountType===type&&String(a.reportSection??'').startsWith(prefix)).map(a=>({...a,amount:normal(a)}));return{section:prefix,lines,total:round(lines.reduce((s,l)=>s+l.amount,0))};}

app.get('/health', (_req:Request,res:Response)=>res.json({ok:true,service:'accounthub-api',modules:['journal-first-ledger','standard-coa','manual-journals','sales-subledger','purchase-subledger','bank-feeds','bank-reconciliation','ocr','mtd-vat','tenant-isolation','payroll-posting','reports']}));
app.get('/api/bookkeeping/chart-template/uk-limited-company',(_req,res)=>res.json(coa));
...
app.post('/api/ledger/equation/verify',(req,res)=>{const accounts=Array.isArray(req.body.accounts)?req.body.accounts:[];const assets=round(accounts.filter((a:any)=>a.accountType==='asset').reduce((s:number,a:any)=>s+Number(a.debit??0)-Number(a.credit??0),0));const liabilities=round(accounts.filter((a:any)=>a.accountType==='liability').reduce((s:number,a:any)=>s+Number(a.credit??0)-Number(a.debit??0),0));const equity=round(accounts.filter((a:any)=>a.accountType==='equity').reduce((s:number,a:any)=>s+Number(a.credit??0)-Number(a.debit??0),0));const income=round(accounts.filter((a:any)=>a.accountType==='income').reduce((s:number,a:any)=>s+Number(a.credit??0)-Number(a.debit??0),0));const expenses=round(accounts.filter((a:any)=>a.accountType==='expense').reduce((s:number,a:any)=>s+Number(a.debit??0)-Number(a.credit??0),0));const retainedEarningsMovement=round(income-expenses);const rightSide=round(liabilities+equity+retainedEarningsMovement);res.json({assets,liabilities,equity,income,expenses,retainedEarningsMovement,rightSide,balanced:assets===rightSide,difference:round(assets-rightSide)});});
app.post('/api/subledgers/sales-invoices/posting-preview',(req,res)=>{const net=Number(req.body.netAmount??0),vat=Number(req.body.vatAmount??0);res.json({source:'sales_invoice',reference:req.body.invoiceNumber,journal:{lines:[{accountCode:req.body.debtorAccount??'3300',debit:round(net+vat),credit:0},{accountCode:req.body.salesAccount??'1000',debit:0,credit:net},{accountCode:req.body.vatAccount??'4100',debit:0,credit:vat}]}});});
app.post('/api/subledgers/purchase-bills/posting-preview',(req,res)=>{const net=Number(req.body.netAmount??0),vat=Number(req.body.vatAmount??0);res.json({source:'purchase_bill',reference:req.body.billNumber,journal:{lines:[{accountCode:req.body.expenseAccount,debit:net,credit:0},{accountCode:req.body.vatAccount??'4100',debit:vat,credit:0},{accountCode:req.body.creditorAccount??'4000',debit:0,credit:round(net+vat)}]}});});
app.post('/api/subledgers/payroll/posting-preview',(req,res)=>{const b=req.body;res.json({source:'payroll_run',journal:{lines:[{accountCode:'2300',debit:Number(b.grossWages??0),credit:0},{accountCode:'2310',debit:Number(b.employerNi??0),credit:0},{accountCode:'2320',debit:Number(b.employerPension??0),credit:0},{accountCode:'4200',debit:0,credit:round(Number(b.employeePaye??0)+Number(b.employeeNi??0)+Number(b.employerNi??0))},{accountCode:'4000',debit:0,credit:round(Number(b.employeePension??0)+Number(b.employerPension??0))},{accountCode:'4000',debit:0,credit:Number(b.netPay??0)}]}});});
app.post('/api/reports/profit-and-loss',(req,res)=>{const accounts=Array.isArray(req.body.accounts)?req.body.accounts:[];const income=accounts.filter((a:any)=>a.accountType==='income').map((a:any)=>({...a,amount:normal(a)}));const expenses=accounts.filter((a:any)=>a.accountType==='expense').map((a:any)=>({...a,amount:normal(a)}));const totalIncome=round(income.reduce((s:number,a:any)=>s+a.amount,0));const totalExpenses=round(expenses.reduce((s:number,a:any)=>s+a.amount,0));res.json({income,expenses,totalIncome,totalExpenses,netProfit:round(totalIncome-totalExpenses)});});
