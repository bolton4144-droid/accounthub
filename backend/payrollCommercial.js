...
const menuGroups = [
  { name:'Pay', items:['Pay Details','Employer RTI schedule','Employee payslip','Employer summary current period','Employer summary tax period','Employer payslip P30','Pension monthly summary','Year-to-date figures','Payments','Money manager entries','Accounting file','Tax & NIC actually paid','Print and post','Employer notes'] },
  { name:'Forms', items:['P11 deductions worksheet','P60 end of year','Employer end-of-year summary','P11D expenses and benefits','P11D(b)','P45 leaving statement','Blank new employee form'] },
  { name:'Analysis', items:['Employee pay totals','Period totals','Department analysis','Pension contributions','Accounts reconciliation','Annual and average pay','Pay count report','National minimum wage check','Statutory pay recovery','HMRC funding report','Employer NIC allowance','Apprenticeship levy','Class 1A NIC summary','Data validation','RTI validation','PDF password check','Tax & NIC rates','Pay elements report'] },
  { name:'Employees', items:['Employee details','Add new employee','Expenses & benefits','Calendar','Employee list','Employee count','Joiners and leavers'] },
  { name:'Employer', items:['Employer details','Departments','Statutory pay funding','Tax refund funding','CIS suffered','Previous year overpayment to HMRC'] },
  { name:'Pensions', items:['Pension scheme details','Assessment','Employee pensions report','Employee pension rates','Pensions validation','Generate upload files','Letters','Setup guides'] },
  { name:'Agent', items:['Agent details','RTI batch processor','Invoice','Payslip count','Pay schedule','New employee form','New subcontractor form','New employer form','Online authority FBI2','Employer list','Employer payslip count','Save client transfer backup'] },
  { name:'CIS', items:['Subcontractors','Monthly return','Verification','CIS suffered','Deduction statements'] },
  { name:'Tools', items:['Setup','Mid-year start','Pay dates','Online filing IDs & passwords','Download PAYE notices','Email','Print payslips for more than one pay period','Calculator','Address labels','HMRC audit file','Import data','Export data','Run money manager'] },
  { name:'Help', items:['Program guide','Help and support','Search help and support','Quick start guide','End of year guide','Program update','New features and guides','About AccountHub Payroll','User licence'] }
];
...
function periodTotals(employerId = employers[0].id) {
  const rows = employees.filter((e) => e.employerId === employerId).map((employee) => {
    const slip = calculatePay(employee.id);
    return {
      ...
      taxable: slip.totals.taxable,
      paye: Number(slip.deductions.find((x) => x[0] === 'PAYE tax')?.[1] || 0),
      employeeNic: Number(slip.deductions.find((x) => x[0] === 'Employee NIC')?.[1] || 0),
      employerNic: Number(slip.employerCosts.find((x) => x[0] === 'Employer NIC')?.[1] || 0),
      employeePension: Number(slip.deductions.find((x) => x[0] === 'Pension')?.[1] || 0),
      employerPension: Number(slip.employerCosts.find((x) => x[0] === 'Employer pension')?.[1] || 0),
      net: slip.totals.net
    };
  });
  const totals = rows.reduce((a, r) => {
    ['gross','taxable','paye','employeeNic','employerNic','employeePension','employerPension','net'].forEach((k) => { a[k] = round((a[k] || 0) + Number(r[k] || 0)); });
    return a;
  }, {});
  const departments = rows.reduce((a, r) => {
    const key = r.department || 'Unassigned';
    a[key] = a[key] || { department:key, headcount:0, gross:0, net:0, employerCost:0 };
    a[key].headcount += 1;
    a[key].gross = round(a[key].gross + r.gross);
    a[key].net = round(a[key].net + r.net);
    a[key].employerCost = round(a[key].employerCost + r.gross + r.employerNic + r.employerPension);
    return a;
  }, {});
  return { employerId, period:'Mar-2026', rows, totals, departments:Object.values(departments) };
}

function payrollValidation(employerId = employers[0].id) {
  const employer = employers.find((e) => e.id === employerId) || employers[0];
  const checks = [];
  const add = (id, label, status, detail, owner = 'Payroll') => checks.push({ id, label, status, detail, owner });
  add('paye-reference', 'PAYE reference configured', employer.setup.payeReferenceDistrict && employer.setup.payeReferenceSuffix ? 'pass' : 'fail', 'Required before RTI submission.', 'Employer setup');
  add('accounts-office', 'Accounts office reference configured', employer.setup.accountsOfficeReference ? 'pass' : 'fail', 'Required for PAYE payment schedules.', 'Tax office');
  add('pdf-password', 'PDF password policy', employer.setup.pdfPassword ? 'pass' : 'warning', 'Payslips can be generated, but password protection is not configured.', 'Documents');
  add('unique-payroll-id', 'Unique payroll IDs', new Set(employees.map((e) => e.payrollId)).size === employees.length ? 'pass' : 'fail', 'Payroll IDs must be unique across the employer.', 'Employees');
  add('bank-bacs', 'Employer BACS bank details', employer.setup.accountNumber && employer.setup.sortCode ? 'pass' : 'warning', 'Needed for payment files and optional RTI bank details.', 'Bank');
  add('pension-provider', 'Pension provider selected', employer.setup.pensionProvider ? 'pass' : 'warning', 'Provider setup is needed before upload files are produced.', 'Pensions');
  add('hmrc-rti', 'HMRC RTI live filing', 'gated', 'Blocked until HMRC credentials, test scenarios, and product recognition are completed. No certification is claimed.', 'Compliance');
  employees.forEach((employee) => {
    const p = payRuns[employee.id];
    const hourly = Number(p.rate1 || 0) && Number(p.hours1 || 0) ? Number(p.rate1) : null;
    add(`nmw-${employee.id}`, `NMW check - ${employee.firstName} ${employee.lastName}`, hourly && hourly < 12.21 ? 'warning' : 'pass', hourly ? `Rate 1 is GBP ${hourly.toFixed(2)}.` : 'No hourly rate requiring NMW check in this period.', 'Pay details');
    add(`starter-${employee.id}`, `Starter/P45 completeness - ${employee.firstName} ${employee.lastName}`, employee.starterStatement || employee.p45PreviousPay ? 'pass' : 'warning', 'Starter statement or P45 values should be recorded for new starters.', 'Employees');
  });
  const summary = checks.reduce((a, c) => { a[c.status] = (a[c.status] || 0) + 1; return a; }, {});
  return { employerId, generatedAt:new Date().toISOString(), summary, checks };
}

function pensionUploadPreview(provider = 'NEST') {
  const rows = employees.map((employee) => {
    ...
    return {
      ...
      pensionablePay: p.periodGross,
      employeeContribution: p.employeePensionDeduction,
      employerContribution: round(p.periodGross * Number(p.employerPensionPercent || 0) / 100),
      fileStatus: provider === 'NEST' ? 'preview_ready_credentials_required' : 'provider_mapping_required'
    };
  });
  return { provider, format: provider === 'Smart PAPDIS' ? 'PAPDIS-compatible preview' : 'Provider upload preview', rows, credentialsRequired:true };
}

function reportPayload(reportId, employeeId, employerId) {
  const totals = periodTotals(employerId);
  const payslip = payslipFor(employeeId || employees[0].id, 'detailed-accountant');
  const validation = payrollValidation(employerId);
  const pensionRows = pensionUploadPreview(employers[0].setup.pensionProvider).rows;
  const reportRows = {
    'employee-pay-totals': totals.rows,
    'period-totals': [totals.totals],
    'department-analysis': totals.departments,
    'pension-contributions': pensionRows,
    'pension-monthly-summary': pensionRows,
    'accounts-reconciliation': [{ account:'Wages and salaries', debit:totals.totals.gross, credit:0 }, { account:'PAYE / Payroll Taxes', debit:0, credit:round(totals.totals.paye + totals.totals.employeeNic + totals.totals.employerNic) }, { account:'Pension creditor', debit:0, credit:round(totals.totals.employeePension + totals.totals.employerPension) }, { account:'Net wages control', debit:0, credit:totals.totals.net }],
    'pay-count-report': [{ period:'Mar-2026', employees:totals.rows.length, locked:periods.find((p) => p.name === 'Mar-2026')?.status === 'locked' }],
    'data-validation': validation.checks,
    'rti-validation': validation.checks.filter((c) => c.status === 'fail' || c.status === 'gated'),
    'pdf-password-check': validation.checks.filter((c) => c.id === 'pdf-password'),
    'tax-nic-rates': [{ band:'PAYE basic rate demo architecture', rate:'20%', status:'replace with live tax-year tables before certification' }, { band:'NI employee main demo architecture', rate:'8%', status:'replace with live HMRC fixtures before certification' }, { band:'NI employer secondary demo architecture', rate:'13.8%', status:'replace with live HMRC fixtures before certification' }],
    'pay-elements-report': Object.values(payRuns).map((p) => ({ employeeId:p.employeeId, basic:p.monthlySalary, hourly:p.hourlyTotal, bonus:p.bonus, statutory:round(Number(p.sspAmount) + Number(p.smpAmount) + Number(p.sppAmount) + Number(p.sapAmount) + Number(p.shppAmount)), deductions:round(Number(p.attachmentOfEarnings) + Number(p.loanRepayment) + Number(p.payrollGiving)) }))
  };
  return { totals, payslip, validation, rows: reportRows[reportId] || totals.rows };
}

function bootstrap() {
  return { employers, employerSetupTabs, employees, employeeFields:['employee details','starter form','P45 starter values','bank/BACS','tax code','NI number/category','department','student loan','director flag','irregular payment','pension settings'], payDetailTabs, payRuns, periods, reportActions, menuGroups, pensionProviders, payslipTemplates, p32:p32Summary(), validation:payrollValidation(), periodTotals:periodTotals(), rti:{ status:'gated', liveFilingEnabled:false, blockers:['HMRC credentials not configured','HMRC RTI product certification not configured'], notice:'No live HMRC RTI filing is enabled or claimed.' }, auditLog };
}
...
:root{--p:#579a76;--d:#101815;--m:#66736c;--l:#dce6e0;--bg:#f5f8f6;--warn:#9a6a1d;--fail:#a23838}*{box-sizing:border-box}body{margin:0;background:var(--bg);font-family:Inter,system-ui,Segoe UI,sans-serif;color:var(--d);overflow:hidden}.app{height:100vh;display:grid;grid-template-rows:34px 56px 1fr}.menubar{display:flex;gap:4px;align-items:center;background:white;border-bottom:1px solid var(--l);padding:4px 10px;overflow:auto}.menu{position:relative}.menu>button{border:0;background:white;border-radius:5px;padding:5px 8px;font-weight:800;cursor:pointer}.menu>button:hover{background:#edf5f0}.dropdown{display:none;position:absolute;top:29px;left:0;z-index:20;min-width:250px;max-height:70vh;overflow:auto;background:white;border:1px solid var(--l);box-shadow:0 16px 40px rgba(16,24,21,.14);border-radius:8px;padding:6px}.menu:hover .dropdown{display:block}.dropitem{display:block;width:100%;border:0;background:white;text-align:left;padding:8px 10px;border-radius:5px;cursor:pointer;font:inherit;font-size:13px}.dropitem:hover{background:#edf5f0}.top{display:grid;grid-template-columns:1.2fr .7fr .7fr .8fr auto auto auto auto;gap:8px;align-items:center;padding:8px 12px;background:white;border-bottom:1px solid var(--l)}select,input,textarea{border:1px solid var(--l);border-radius:6px;min-height:34px;padding:0 8px;font:inherit;font-size:13px;background:white}textarea{padding:8px;min-height:70px}.btn{border:1px solid var(--l);background:white;border-radius:6px;min-height:34px;padding:0 10px;font-weight:800;cursor:pointer}.btn.p{background:var(--p);border-color:var(--p);color:white}.body{min-height:0;display:grid;grid-template-columns:235px 1fr 345px}.rail,.main,.side{min-height:0;overflow:auto;border-right:1px solid var(--l);background:white}.rail{padding:10px}.main{background:var(--bg);display:grid;grid-template-rows:44px 1fr}.side{padding:10px;border-right:0;border-left:1px solid var(--l)}.tabs{display:flex;gap:6px;align-items:center;overflow:auto;padding:6px 10px;background:white;border-bottom:1px solid var(--l)}.tab{border:1px solid var(--l);background:white;border-radius:6px;padding:8px 10px;font-size:12px;font-weight:800;white-space:nowrap;cursor:pointer}.tab.active{background:var(--p);color:white;border-color:var(--p)}.panel{margin:10px;background:white;border:1px solid var(--l);border-radius:8px;padding:12px}.grid{display:grid;grid-template-columns:repeat(3,minmax(160px,1fr));gap:10px}.setup-grid{display:grid;grid-template-columns:repeat(4,minmax(160px,1fr));gap:10px}.card{border:1px solid var(--l);border-radius:8px;background:#fbfdfc;padding:10px;margin-bottom:8px;cursor:pointer}.card.active{border-color:var(--p);box-shadow:inset 4px 0 0 var(--p)}.card strong,.card span,.card small{display:block}.card span,.muted{color:var(--m);font-size:12px}.split{display:grid;grid-template-columns:1fr 1fr;gap:10px}h2,h3{margin:0 0 10px}.pill{border-radius:999px;background:#e8f4ee;color:#3f7358;padding:4px 8px;font-size:12px;font-weight:900}.pill.warning{background:#fff2d8;color:var(--warn)}.pill.fail{background:#ffe8e8;color:var(--fail)}.pill.gated{background:#edf0f3;color:#59646f}.summary{display:flex;justify-content:space-between;gap:10px;border-bottom:1px solid var(--l);padding:7px 0}.reports{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.report{border:1px solid var(--l);border-radius:6px;padding:9px;background:#fbfdfc;font-size:12px;cursor:pointer}.report.ready{border-color:var(--p)}.preview{border:1px solid var(--l);border-radius:8px;background:#fbfdfc;margin-top:10px;padding:10px;max-height:360px;overflow:auto}.check{border:1px solid var(--l);border-left:5px solid var(--p);border-radius:7px;padding:8px;margin:7px 0;background:white}.check.warning{border-left-color:var(--warn)}.check.fail{border-left-color:var(--fail)}.check.gated{border-left-color:#59646f}table{width:100%;border-collapse:collapse}th,td{text-align:left;border-bottom:1px solid var(--l);padding:8px;font-size:12px}.money{text-align:right}.payslip{border:1px solid var(--l);background:white;border-radius:8px;padding:12px;margin-bottom:10px}.payslip.detailed-accountant{box-shadow:inset 0 5px 0 var(--p)}.payslip.compact{font-size:12px}.payslip.white-label{border-color:#111}.payslip.modern-branded{background:linear-gradient(180deg,#fff,#f6fbf8)}@media(max-width:1200px){body{overflow:auto}.app{height:auto}.body,.top,.grid,.setup-grid,.reports{grid-template-columns:1fr}.main{display:block}}</style></head><body><div class="app"><div class="menubar" id="menubar"></div><div class="top"><select id="employer"></select><select id="taxYear"></select><select id="frequency"></select><select id="period"></select><button class="btn p" id="save">Save</button><button class="btn" id="calc">Calculate</button><button class="btn" id="preview">Preview payslip</button><button class="btn" id="lock">Lock period</button></div><div class="body"><div class="rail"><h3>Employees</h3><div id="employees"></div><h3>Actions</h3><div id="quickReports"></div></div><div class="main"><div class="tabs" id="primaryTabs"></div><div id="content"></div></div><div class="side"><h3>Payslip</h3><select id="template"></select><div id="payslip"></div><h3>P32</h3><div id="p32"></div><h3>Compliance</h3><div id="rti"></div></div></div></div><script>
...
function reportId(name){return name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}
function renderMenus(){menubar.innerHTML=S.menuGroups.map(g=>'<div class="menu"><button>'+g.name+'</button><div class="dropdown">'+g.items.map(i=>'<button class="dropitem" data-menu-report="'+reportId(i)+'">'+i+'</button>').join('')+'</div></div>').join('')}
function htmlTable(rows){if(!rows||!rows.length)return '<p class="muted">No rows for this report yet.</p>';const cols=Object.keys(rows[0]);return '<table><tr>'+cols.map(c=>'<th>'+c+'</th>').join('')+'</tr>'+rows.map(r=>'<tr>'+cols.map(c=>'<td class="'+(typeof r[c]==='number'?'money':'')+'">'+(typeof r[c]==='number'?money(r[c]):r[c])+'</td>').join('')+'</tr>').join('')+'</table>'}
function renderReportPreview(x){const rows=(x.data&&x.data.rows)||[];content.innerHTML='<div class="panel"><h2>'+x.title+'</h2><span class="pill '+x.status+'">'+x.status+'</span><p class="muted">'+(x.status==='gated_compliance_state'?'This workflow is held behind credentials, audit evidence, and HMRC/product recognition.':'Generated from the active payroll dataset.')+'</p><div class="preview">'+htmlTable(rows)+'</div></div>'}
function renderValidation(){const v=S.validation;return '<div class="panel"><h2>Payroll validation</h2><div class="split">'+['pass','warning','fail','gated'].map(k=>'<div class="summary"><span>'+k+'</span><strong>'+(v.summary[k]||0)+'</strong></div>').join('')+'</div>'+v.checks.map(c=>'<div class="check '+c.status+'"><strong>'+c.label+'</strong><br><span class="muted">'+c.owner+' | '+c.detail+'</span></div>').join('')+'</div>'}
function renderShell(){employer.innerHTML=S.employers.map(e=>'<option value="'+e.id+'">'+e.name+'</option>').join('');taxYear.innerHTML=['2025-26','2026-27'].map(x=>'<option>'+x+'</option>').join('');frequency.innerHTML=['Monthly','Weekly','Fortnightly','Four-weekly'].map(x=>'<option>'+x+'</option>').join('');period.innerHTML=S.periods.map(p=>'<option>'+p.name+' - '+p.status+'</option>').join('');template.innerHTML=S.payslipTemplates.map(t=>'<option value="'+t.id+'">'+t.name+'</option>').join('');activeEmployee=S.employees[0].id;primaryTabs.innerHTML=['Employer Setup','Employee Records','Pay Details','Reports','Pensions','Payslips'].map(t=>'<button class="tab '+(t===primary?'active':'')+'" data-primary="'+t+'">'+t+'</button>').join('');renderMenus();renderEmployees();renderMain();renderSide()}
...
function renderReports(){content.innerHTML='<div class="panel"><h2>Payroll reports and forms</h2><div class="reports">'+S.reportActions.map(r=>'<div class="report '+r.status+'" data-report="'+r.id+'"><strong>'+r.name+'</strong><br><span class="muted">'+(r.gated?'Gated/compliance checked':'Working screen')+'</span></div>').join('')+'</div></div>'+renderValidation()}
function renderPensions(){content.innerHTML='<div class="panel"><h2>Pensions</h2><div class="reports">'+S.pensionProviders.map(p=>'<div class="report" data-provider="'+p+'"><strong>'+p+'</strong><br><span class="muted">Scheme details, assessment, validation and upload file setup guide</span></div>').join('')+'</div></div><div class="panel"><h2>Current pension upload preview</h2>'+htmlTable((S.periodTotals.rows||[]).map(r=>({employee:r.employee, employeeContribution:r.employeePension, employerContribution:r.employerPension, status:'preview'})))+'</div>'}
...
function openReport(reportId){api('/api/payroll/commercial/report',{method:'POST',body:JSON.stringify({reportId,employeeId:activeEmployee,employerId:employer.value})}).then(renderReportPreview)}
primaryTabs.onclick=e=>{if(e.target.dataset.primary){primary=e.target.dataset.primary;secondary='';renderMain()}};
menubar.onclick=e=>{if(e.target.dataset.menuReport){const id=e.target.dataset.menuReport;const has=S.reportActions.find(r=>r.id===id);if(has){primary='Reports';renderMain();openReport(id)}else{content.innerHTML='<div class="panel"><h2>'+e.target.textContent+'</h2><span class="pill gated">workflow scaffolded</span><p class="muted">This commercial workflow is represented in navigation and will be backed by a dedicated route when its compliance/data dependency is enabled.</p></div>'}}};
content.onclick=e=>{if(e.target.dataset.secondary){secondary=e.target.dataset.secondary;renderMain()}if(e.target.dataset.report){openReport(e.target.dataset.report)}if(e.target.dataset.provider){api('/api/payroll/commercial/pensions/upload-preview/'+encodeURIComponent(e.target.dataset.provider)).then(x=>{content.innerHTML='<div class="panel"><h2>'+x.provider+' upload preview</h2><span class="pill warning">'+x.format+'</span><p class="muted">Credentials are required before a live file can be submitted or transmitted.</p><div class="preview">'+htmlTable(x.rows)+'</div></div>'})}};
employees.onclick=e=>{const c=e.target.closest('[data-ee]');if(c){activeEmployee=c.dataset.ee;renderEmployees();renderMain();renderSide()}};
template.onchange=renderSide;
save.onclick=async()=>{if(primary==='Employer Setup'){collectFields(currentEmp().setup);await api('/api/payroll/commercial/employers/'+employer.value+'/setup',{method:'PUT',body:JSON.stringify(currentEmp().setup)})}else if(primary==='Employee Records'){collectFields(currentEmployee());await api('/api/payroll/commercial/employees/'+activeEmployee,{method:'PUT',body:JSON.stringify(currentEmployee())})}else{collectFields(currentRun());await api('/api/payroll/commercial/payruns/'+activeEmployee,{method:'PUT',body:JSON.stringify(currentRun())})}S=await api('/api/payroll/commercial/bootstrap');renderMain();renderSide()};
calc.onclick=async()=>{await save.onclick();const x=await api('/api/payroll/commercial/calculate',{method:'POST',body:JSON.stringify({employeeId:activeEmployee})});S.payRuns[activeEmployee]=x.payRun;S.p32=x.p32;S.validation=(await api('/api/payroll/commercial/validation')).validation;renderMain();renderSide()};
preview.onclick=()=>{primary='Payslips';renderMain();renderSide()};
lock.onclick=async()=>{const x=await api('/api/payroll/commercial/period/'+employer.value+'/lock',{method:'POST',body:JSON.stringify({period:period.value})});S=await api('/api/payroll/commercial/bootstrap');renderShell();content.innerHTML='<div class="panel"><h2>Period locked</h2><p class="muted">'+x.period+' is locked for amendment control. Submission remains not submitted.</p></div>'};
api('/api/payroll/commercial/bootstrap').then(d=>{S=d;renderShell()});
</script></body></html>`;
}
...
router.get('/api/payroll/commercial/validation', (req, res) => res.json({ validation:payrollValidation(req.query.employerId || employers[0].id) }));
router.get('/api/payroll/commercial/period/:employerId/totals', (req, res) => res.json(periodTotals(req.params.employerId)));
router.get('/api/payroll/commercial/pensions/upload-preview/:provider', (req, res) => res.json(pensionUploadPreview(req.params.provider)));
router.post('/api/payroll/commercial/report', (req, res) => { const r = reportActions.find((x) => x.id === req.body.reportId); if (!r) return res.status(404).json({ error:'Report not found' }); res.json({ id:r.id, title:r.name, status:r.gated ? 'gated_compliance_state' : 'ready', data:{ employerId:req.body.employerId, employeeId:req.body.employeeId, p32:p32Summary(), ...reportPayload(r.id, req.body.employeeId || employees[0].id, req.body.employerId || employers[0].id) } }); });
