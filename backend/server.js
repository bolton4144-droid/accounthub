...
function reportSection(accounts, prefix, accountType) {
...
app.get('/', (_req, res) => res.json({ service: 'accounthub-api', health: '/health', docs: '/api/bookkeeping/chart-template/uk-limited-company' }));
...
app.post('/api/reports/profit-and-loss', (req, res) => {
  const accounts = Array.isArray(req.body.accounts) ? req.body.accounts : [];
  const income = accounts.filter((a) => a.accountType === 'income').map((a) => ({ ...a, amount: normalBalance(a) }));
  const expenses = accounts.filter((a) => a.accountType === 'expense').map((a) => ({ ...a, amount: normalBalance(a) }));
...
app.post('/api/reports/balance-sheet/structured', (req, res) => {
  const accounts = Array.isArray(req.body.accounts) ? req.body.accounts : [];
  const fixedAssets = reportSection(accounts, 'fixed_assets', 'asset');
  const currentAssets = reportSection(accounts, 'current_assets', 'asset');
  const currentLiabilities = reportSection(accounts, 'current_liabilities', 'liability');
  const longTermLiabilities = reportSection(accounts, 'long_term_liabilities', 'liability');
  const capitalAndReserves = reportSection(accounts, 'capital_and_reserves', 'equity');
